...
function l(t) {
  ...
  (e && (e.innerHTML = t.items.map(T).join("")),
...
function C() {
  (document.addEventListener("click", async (t) => {
    ...
    document.addEventListener("submit", async (t) => {
      ...
      try {
        const i =
          (a = e.querySelector('[name="id"]')) == null ? void 0 : a.value;
        if (!i) throw new Error("Add to cart: variant ID not found");
        const s = parseInt(
        ...
        await M([{ id: i, quantity: s }]);
        const d = await S();
        (l(d), q());
      } catch (i) {
        console.error("Add to cart failed:", i);
      } finally {
        (r && (r.textContent = n), (o = !1));
      }
    }),
    document.addEventListener("click", async (t) => {
      ...
      if (!n || !a) return;
      const c = Math.max(0, parseInt(a.value, 10) - 1);
      o = !0;
      try {
        const i = await g(n, c);
        l(i);
...
function _() {
  ...
  (new IntersectionObserver(
    ...
    r.addEventListener("click", () => {
      ...
      if (i) {
        const d = i.querySelector("option:not([disabled]):checked") !== null;
  ...
  a &&
    a.addEventListener("change", () => {
      const i = !a.options[a.selectedIndex].disabled;
      ((r.disabled = !i),
...
function R() {
  ...
  t &&
    (document.addEventListener("click", async (r) => {
      ...
      if (!n || (r.preventDefault(), y)) return;
      ((y = !0), (u = n));
      ...
      try {
        const i = await O(a),
          ...
          d = s.querySelector("main") ?? s.body;
        c && (c.innerHTML = d.innerHTML);
      } catch {
        c &&
          (c.innerHTML =
    ...
    document.addEventListener("change", (r) => {
      ...
      if (!n || t.getAttribute("aria-hidden") === "true") return;
      const a = t.querySelector("[data-product-variants]");
      if (!a) return;
      let c;
      try {
        c = JSON.parse(a.textContent);
      } catch {
        return;
      }
      const i = c.find((f) => String(f.id) === n.value);
      if (!i) return;
      const s = t.querySelector("[data-quick-view-price]");
      if (s) {
        const f = x(i.price),
          v = i.compare_at_price;
        v && v > i.price
          ? (s.innerHTML = `<span class="text-red-600 font-semibold">${f}</span> <s class="text-gray-400 text-sm">${x(v)}</s>`)
          : (s.innerHTML = `<span class="font-semibold text-gray-900">${f}</span>`);
      }
      const d = t.querySelector("[data-add-to-cart]");
      d &&
        ((d.disabled = !i.available),
        d.setAttribute("aria-disabled", String(!i.available)),
        (d.textContent = i.available ? "Add to cart" : "Sold out"));
    }));
}
function F() {
  ...
  t.length &&
    t.forEach((e) => {
      const r = e.querySelector("[data-accordion-trigger]"),
        n = e.querySelector("[data-accordion-panel]");
      !r ||
        !n ||
        r.addEventListener("click", () => {
...
function N(t, e, r) {
  ...
  (r.classList.add("hidden"), (e.innerHTML = t.map(Q).join("")));
}
function V(t, e) {
...
function J(t, e, r) {
  ...
  const n = A(r);
  if (!n.length) return;
  const a = n.findIndex((c) => c.hasAttribute("data-active"));
  if (t.key === "ArrowDown") {
    t.preventDefault();
    const c = a < n.length - 1 ? a + 1 : 0;
    k(n[c], e, r);
  } else if (t.key === "ArrowUp") {
    t.preventDefault();
    const c = a > 0 ? a - 1 : n.length - 1;
    k(n[c], e, r);
  } else if (t.key === "Enter" && a !== -1) {
    t.preventDefault();
    const c = n[a].querySelector("a");
...
function X() {
  ...
  const e = t.querySelector("[data-predictive-search-input]"),
    ...
    n = t.querySelector("[data-predictive-search-list]"),
    a = t.querySelector("[data-predictive-search-empty]");
  !e ||
    !r ||
    !n ||
    !a ||
    ...
    e.addEventListener("keydown", (c) => J(c, e, r)),
...
function B() {
  const t = document.querySelector("[data-sort-select]");
  t &&
    t.addEventListener("change", () => {
...
function W() {
  const t = document.querySelector("[data-filter-open]"),
    e = document.querySelector("[data-filter-close]"),
  ...
  (t.addEventListener("click", a),
    ...
    document.addEventListener("keydown", (i) => {
      i.key === "Escape" && !r.classList.contains("-translate-x-full") && c();
    }));
}
function G() {
  const t = document.querySelector("[data-product-recommendations]");
  if (!t) return;
  const { productId: e, url: r, limit: n = 4 } = t.dataset;
  fetch(`${r}?section_id=product-recommendations&product_id=${e}&limit=${n}`)
    ...
    .then((a) => {
      ...
      i != null && i.children.length
        ? (t.innerHTML = i.innerHTML)
        : (s = t.closest("section")) == null || s.remove();
    })
    .catch(() => {
      var a;
      return (a = t.closest("section")) == null ? void 0 : a.remove();
    });
}
document.addEventListener("DOMContentLoaded", () => {
  (C(),
    ...
    z(),
    G(),
    document.querySelectorAll("[data-date-min-today]").forEach((t) => {
