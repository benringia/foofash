  ...
  return `
    <li class="flex gap-4 py-4 border-b border-gray-100 last:border-0" data-line-item data-item-key="${t.key}">
      ...
      <div class="flex flex-1 flex-col gap-1 min-w-0">
        ...
        ${r}
        <p class="text-sm font-semibold text-gray-900">${S(t.final_line_price)}</p>
...
function s(t) {
  const e = document.querySelector("[data-cart-items]"),
    ...
    c = document.querySelectorAll("[data-cart-count]");
  (e && (e.innerHTML = t.items.map(A).join("")),
    r && r.classList.toggle("hidden", t.item_count > 0),
    a.forEach((n) => {
      n.textContent = S(t.total_price);
    }),
    c.forEach((n) => {
      n.textContent = t.item_count;
    }));
}
function b() {
...
function I() {
  (document.addEventListener("click", async (t) => {
    if (
      ...
      ((f = t.target.closest("[data-cart-toggle]")), !i)
    ) {
      i = !0;
      try {
        const e = await g();
    ...
    document.addEventListener("submit", async (t) => {
      ...
      try {
        const o =
            (c = e.querySelector('[name="id"]')) == null ? void 0 : c.value,
          p = parseInt(
            ((n = e.querySelector('[name="quantity"]')) == null
              ? void 0
              : n.value) ?? "1",
            10,
          );
        await L([{ id: o, quantity: p }]);
        const d = await g();
        (s(d), b());
      } catch (o) {
        console.error("Add to cart failed:", o);
      } finally {
        (r && (r.textContent = a), (i = !1));
      }
    }),
    document.addEventListener("click", async (t) => {
      ...
      const r = e.closest("[data-line-item]"),
        ...
        c = r == null ? void 0 : r.querySelector("[data-quantity-input]");
      if (!a || !c) return;
      const n = Math.max(0, parseInt(c.value, 10) - 1);
      i = !0;
      try {
        const o = await v(a, n);
        s(o);
      } catch (o) {
        console.error("Quantity update failed:", o);
      } finally {
        i = !1;
      }
    }),
    ...
    document.addEventListener("click", async (t) => {
      ...
      if (a) {
        ...
        try {
          ...
          console.error("Remove item failed:", c);
        } finally {
          i = !1;
        }
      }
...
function C() {
  ...
  (new IntersectionObserver(
    ([n]) => {
      ...
      (e.classList.toggle("translate-y-full", o),
    ...
    r.addEventListener("click", () => {
      ...
      if (!n) return;
      const o = n.querySelector("[data-variant-selector]");
      if (o) {
        const d = o.querySelector("option:not([disabled]):checked") !== null;
        if (((r.disabled = !d), !d)) return;
      }
      n.requestSubmit(t);
    }));
  const c = document.querySelector("[data-variant-selector]");
  c &&
    c.addEventListener("change", () => {
      const o = !c.options[c.selectedIndex].disabled;
      ((r.disabled = !o),
        r.setAttribute("aria-disabled", String(!o)),
        (r.textContent = o ? "Add to cart" : "Sold out"));
    });
}
let l = !1;
function $() {
...
function T() {
  (document.addEventListener("click", async (t) => {
    ...
    const a = document.querySelector("[data-quick-view-content]");
    (a &&
      (a.innerHTML = '<p class="text-center text-gray-400 py-8">Loading…</p>'),
      $());
    try {
      const c = await M(r),
        n = new DOMParser().parseFromString(c, "text/html"),
        o = n.querySelector("main") ?? n.body;
      a && (a.innerHTML = o.innerHTML);
    } catch {
      a &&
        (a.innerHTML =
    ...
    document.addEventListener("click", (t) => {
      t.target.closest("[data-quick-view-close]") && m();
    }),
    document.addEventListener("click", (t) => {
      t.target.matches("[data-quick-view-overlay]") && m();
    }),
    document.addEventListener("keydown", (t) => {
      if (t.key !== "Escape") return;
      const e = document.querySelector("[data-quick-view-modal]");
      e && e.getAttribute("aria-hidden") === "false" && m();
    }));
}
function j() {
  ...
  t.length &&
    t.forEach((e) => {
      ...
      !r ||
        !a ||
        r.addEventListener("click", () => {
          const c = r.getAttribute("aria-expanded") === "true";
          (r.setAttribute("aria-expanded", String(!c)), (a.hidden = c));
        });
    });
}
let u = null,
...
async function H(t) {
  ...
  const e = new URLSearchParams({
    ...
    r = await fetch(`/search/suggest.json?${e}`, { signal: u.signal });
...
function O(t, e, r) {
  ...
  (r.classList.add("hidden"), (e.innerHTML = t.map(R).join("")));
}
function P(t, e) {
  (e.classList.remove("hidden"), t.setAttribute("aria-expanded", "true"));
}
function h(t, e) {
  (e.classList.add("hidden"), t.setAttribute("aria-expanded", "false"), k(e));
}
function x(t) {
...
function F(t, e, r) {
  ...
  const a = x(r);
  if (!a.length) return;
  const c = a.findIndex((n) => n.hasAttribute("data-active"));
  if (t.key === "ArrowDown") {
    t.preventDefault();
    const n = c < a.length - 1 ? c + 1 : 0;
    q(a[n], e, r);
  } else if (t.key === "ArrowUp") {
    t.preventDefault();
    const n = c > 0 ? c - 1 : a.length - 1;
    q(a[n], e, r);
  } else if (t.key === "Enter" && c !== -1) {
    t.preventDefault();
    const n = a[c].querySelector("a");
    n && (window.location.href = n.href);
  } else t.key === "Escape" && (h(e, r), e.focus());
}
function Q() {
  ...
  const e = t.querySelector("[data-predictive-search-input]"),
    ...
    c = t.querySelector("[data-predictive-search-empty]");
  (e.addEventListener("input", () => {
    ...
    w = setTimeout(async () => {
      try {
        ...
        o.name !== "AbortError" && console.error(o);
      }
    }, 300);
  }),
    e.addEventListener("keydown", (n) => F(n, e, r)),
    document.addEventListener("click", (n) => {
      t.contains(n.target) || h(e, r);
    }));
}
